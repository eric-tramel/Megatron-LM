...
from .param_layout import FullParamLayout, PerBufferParamLayout, pad_param_start, pad_to_divisor
...
class LayerWiseDistributedOptimizer(ChainedOptimizer):
    ...
    def compute_full_param_layout(
        ...
        """Compute parameter layouts for all buffer groups, dispatching per buffer.

        Groups parameters by :class:`BufferKey` via :func:`group_params_for_buffers`
        and chooses a layout per buffer based on the buffer's optimizer routing:

        - ``use_layerwise_distributed_optimizer=True`` buffers (e.g. Muon-managed
          2D weights) use the layerwise shard-aligned size-matching layout, which
          keeps every parameter within a single shard so Newton-Schulz iteration
          can run on whole tensors.
        - ``use_layerwise_distributed_optimizer=False`` buffers (Adam-routed
          biases, norms, embeddings, output) use :class:`DistributedOptimizer`'s
          byte-level layout, which sub-tensor-shards large parameters across DP
          ranks. Adam's update is element-wise so sub-tensor sharding is
          correct, and it avoids the per-shard padding the layerwise scheme
          would otherwise pay for huge embeddings.
    ...
    def _shard_params_from_layout(self, optimizers, full_param_layouts, dp_cp_size, expt_dp_size):
        ...
        Only buffers tagged ``use_layerwise_distributed_optimizer=True`` are
        managed here; non-layerwise buffers (Adam-routed) are managed by a
        separate :class:`DistributedOptimizer` and are skipped.
        """
        ...
        # Map each layerwise-managed param to its shard index.
        ...
        for full_layout in full_param_layouts:
            for buffer_key, layout in full_layout.layouts.items():
                if not buffer_key.use_layerwise_distributed_optimizer:
                    # Adam-routed buffers are sub-tensor-sharded by their own
                    # DistributedOptimizer; skip them here.
                    continue
